<script lang="ts">
	...
	import type { BoardAction } from './types.js';
	...
	let { activeAction, onSelect }: Props = $props();
</script>
...
>
	...
	<ActionButton
		...
		onclick={() => onSelect('add-event-item')}
	>
		{#snippet icon()}
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5">
	...
	<ActionButton
		...
		onclick={() => onSelect('manage-commands')}
	>
		{#snippet icon()}
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5">
